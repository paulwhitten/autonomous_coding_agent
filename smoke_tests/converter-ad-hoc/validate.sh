#!/bin/bash
# Validate ad-hoc smoke test results
#
...
echo "Validating ad-hoc smoke test..."
echo ""
...
# ================================================================
# Extension checks (message 2)
# ================================================================

# Check 7: converter.ts exports kilogramsToPounds
if grep -q "kilogramsToPounds" "$PROJECT_DIR/converter.ts" 2>/dev/null; then
  echo "PASS: converter.ts contains kilogramsToPounds"
  ((PASSED++))
else
  echo "FAIL: converter.ts missing kilogramsToPounds (message 2 not processed?)"
  ((FAILED++))
fi
...
# Check 9: Test file has sufficient test cases (expect >= 8 from msg 1, >= 3 from msg 2)
if [ -f "$PROJECT_DIR/converter.test.ts" ]; then
  # Count lines containing 'expect(' or 'it(' or 'test(' as proxy for test cases
...
if [ -f "$PROJECT_DIR/test_output.txt" ]; then
  ...
  echo "FAIL: test_output.txt not found (agent should capture Jest output)"
  ((FAILED++))
fi

# Check 11: README.md updated (contains converter-related content)
...
if [ -d "$PROJECT_DIR/.git" ]; then
  ...
  # Expect >= 5 commits: 1 setup + 4 from message 1 + 3 from message 2 = 8, but be lenient
  if [ "$COMMIT_COUNT" -ge 5 ]; then
    echo "PASS: Git has $COMMIT_COUNT commits (expected >= 5: 1 setup + agent work)"
    ((PASSED++))
  ...
  # Check: Commits are progressive (different files or content across commits)
...
if [ -f test.log ]; then
  COMPLETED_COUNT=$(grep -c "Work item completed" test.log 2>/dev/null)
  COMPLETED_COUNT=${COMPLETED_COUNT:-0}
  if [ "$COMPLETED_COUNT" -ge 1 ]; then
    echo "PASS: Agent completed $COMPLETED_COUNT work items"
    ((PASSED++))
  else
    echo "FAIL: No work items completed"
    ((FAILED++))
  fi
fi
...
if [ "$FAILED" -eq 0 ]; then
  echo "TEST PASSED — Agent processes ad-hoc messages without a workflow!"
  echo ""
  echo "Evidence:"
  echo "  - No workflow engine configured (pure ad-hoc)"
  echo "  - Source files created with correct exports"
  echo "  - Tests written and passing"
  echo "  - Incremental git commits"
