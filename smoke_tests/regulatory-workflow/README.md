# V-Model Regulatory Evidence Smoke Test (Workflow-Driven Arm)

Workflow-driven arm of the regulatory determinism comparison. A deterministic
workflow engine drives the V-model state machine and performs all git, test
gating, requirement gates, and state sequencing. The LLM agents only produce
content (requirements, code, tests, evidence). This is the determinism arm
that pairs with regulatory-ad-hoc, where the same work is LLM-driven instead.
...
|-------|----------|------|------------------|----------------|
| Manager | smoke-regwf-mgr | manager | Orchestrator (delegation) | None (no workflowFile) |
| Requirements Analyst | smoke-regwf-ra | requirements-analyst | Left top / Right top | REQUIREMENTS_DEFINITION, ACCEPTANCE |
| Developer | smoke-regwf-dev | developer | Bottom | IMPLEMENTING, REWORK, REWORK_ACCEPTANCE |
| QA | smoke-regwf-qa | qa | Right ascending | VERIFICATION |
